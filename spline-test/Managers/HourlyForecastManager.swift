...
//  spline-test
//
//  Created by Viru Repalle on 5/29/24.
//
...
class HourlyForecastManager {
    ...
    func getForecastWeather(latitude: CLLocationDegrees, longitude: CLLocationDegrees) async throws -> ResponseBodyHourly {
        ...
        let decodedData = try JSONDecoder().decode(ResponseBodyHourly.self, from: data)
...
struct ResponseBodyHourly: Decodable {
    var cod: String
    var message: Int
    var cnt: Int
